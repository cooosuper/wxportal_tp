		...
		<table id="functionManage" style="margin: 10px;width: 90%">
			...
			<tr>
				...
				<h3>已有配置</h3>
					<table>
						<tr>
							<td style="background-color: silver">地址</td>
							<td style="background-color: silver">电话</td>
							<td style="background-color: silver">邮箱</td>
							<td style="background-color: silver">传真</td>
						</tr>
						<?php if(is_array($gcontacts)): $i = 0; $__LIST__ = $gcontacts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td><?php echo ($vo["address"]); ?></td>
								<td><?php echo ($vo["telephone"]); ?></td>
								<td><?php echo ($vo["email"]); ?></td>
								<td><?php echo ($vo["faxnumber"]); ?></td>
							</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					</table>
					<hr/>
					<h3>3G站联系方式配置</h3>
					<center><font color="red" id="tip">&nbsp;</font></center>
		 			<form action="<?php echo U($group_name . '/FunctionManage/setGContact');?>" method="post" enctype="multipart/form-data">
						<table>
							<tr>
								<td>地址</td>
								<td>
									<input name="gContactAddress" id="gContactAddress"/>
									<input type="hidden" name="wxaccountid" id="wxaccountid" value="<?php echo ($wxaccount['id']); ?>"/>
								</td>
							</tr>
							<tr>
								<td>电话</td>
								<td><input name="gContactTelephone" id="gContactTelephone"/></td>
							</tr>
							<tr>
								<td>邮箱</td>
								<td><input name="gContactEmail" id="gContactEmail"/></td>
							</tr>
							<tr>
								<td>传真</td>
								<td><input name="gContactFaxNumber" id="gContactFaxNumber"/></td>
							</tr>
							<tr>
