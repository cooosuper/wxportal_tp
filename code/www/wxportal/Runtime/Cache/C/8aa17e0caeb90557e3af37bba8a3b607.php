		...
		<table id="functionManage" style="margin: 10px;width: 90%">
			...
			<tr>
				...
				<h3>已有配置</h3>
					<table>
						<tr>
							<td style="background-color: silver">介绍配图</td>
							<td style="background-color: silver">具体内容</td>
							<td style="background-color: silver">排序</td>
							<td style="background-color: silver">操作</td>
						</tr>
						<?php if(is_array($gdetails)): $i = 0; $__LIST__ = $gdetails;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td><img src="<?php echo ($vo["thumbpicurl"]); ?>"/></td>
								<td><?php echo ($vo["content"]); ?></td>
								<td><?php echo ($vo["sort"]); ?></td>
								<td>
									<input type="button" id="submit_input" 
										onclick="window.location='<?php echo U($group_name . '/FunctionManage/delGDetail?wxaccountid=' . $wxaccount['id'] . '&gDetailId=' . $vo['id']);?>'" value="删除"/>
								</td>
					...
					<center><font color="red" id="tip">&nbsp;</font></center>
		 			<form action="<?php echo U($group_name . '/FunctionManage/setGDetail');?>" method="post" enctype="multipart/form-data">
						<table>
							<tr>
								<td>介绍配图</td>
								<td>
							...
							<tr>
								<td>具体内容</td>
								<td><input name="gDetailContent" id="gDetailContent"/></td>
							</tr>
							<tr>
								<td>排序</td>
								<td><input name="gDetailSort" id="gDetailSort"/></td>
							</tr>
							<tr>
